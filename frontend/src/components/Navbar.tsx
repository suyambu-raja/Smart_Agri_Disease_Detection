...
const Navbar = () => {
  ...
  return (
    <>
      ...
      {user && !isAuthPage && (
        <nav className="fixed bottom-0 left-0 right-0 z-50 bg-[#0f291e] backdrop-blur-md border-t border-[#1e4535] shadow-[0_-4px_20px_rgba(0,0,0,0.1)] pb-[env(safe-area-inset-bottom)]">
          <div className="flex items-center justify-around h-16 pt-2 px-2">
            {navItems.map((item) => {
